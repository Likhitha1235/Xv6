...
// External declaration for proc array
...
swapout(struct proc *p, uint64 va)
{
  ...
  memmove(&swapspace[procidx][sp->offset], (void*)pa, PGSIZE);// writing to the disk .
  release(&swap_lock);
  ...
  *pte = ((sp->offset / PGSIZE) << 10) | (flags & ~PTE_V) | PTE_SWAPPED;
...
swapin(struct proc *p, uint64 va)
{
  ...
  uint64 flags = PTE_FLAGS(*pte);
  *pte = PA2PTE(mem) | (flags & ~PTE_SWAPPED) | PTE_V;
